...
def get_authors(db: Session = Depends(get_db), limit: int = 10, page: int = 1, search: str = ''):
    ...
    authors_query = db.query(models.Author).filter(
        models.Author.full_name.contains(search)).limit(limit).offset(skip)

    authors = []
    for author in authors_query:
        authors.append(author.get_info())
...
def get_author(authorId: str, db: Session = Depends(get_db)):
    ...
    return {"status": "success", "author": author.get_info()}


# [...] edit record
@router.patch('/{authorId}')
def update_author(authorId: str, payload: schemas.AuthorCreateSchema, db: Session = Depends(get_db)):
